...
//    @PreAuthorize("hasAnyAuthority('ADMIN', 'admin')")
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN', 'admin')")
    @PutMapping("/{id}")
    public ApiResponse<UserResponse> update(@PathVariable Long id, @RequestBody UserRequest req) {
...
//    @PreAuthorize("hasAnyAuthority('ADMIN', 'admin')")
    ...
    public ApiResponse<Page<UserResponse>> filter(
        ...
        return ApiResponse.ok(service.filter(username, email, role, status, page, size));
    }
}